...
namespace zx
{
	...
	{
		using ChronoType = T;
		[[nodiscard]] inline duration() noexcept : pm_Data(0) {}
		[[nodiscard]] explicit duration(float count) noexcept : pm_Data(count) {}
		[[nodiscard]] explicit duration(int count) noexcept : pm_Data(count) {}

		[[nodiscard]] duration(const T& copy) : pm_Data(copy) {}

		[[nodiscard]] inline float count() const { return pm_Data.count(); }
		[[nodiscard]] inline duration operator+(const duration& other) const noexcept{
			return duration(pm_Data.count() + other.pm_Data.count());
		}
		[[nodiscard]] inline duration operator-(const duration& other) const noexcept {
			return duration(pm_Data.count() - other.pm_Data.count());
		}
		[[nodiscard]] inline duration operator*(float other) const noexcept {
		...
		inline duration& operator+=(const duration& other) noexcept {
		...
		inline duration& operator*=(float other) noexcept {
		...
		[[nodiscard]] inline bool operator>(const duration& other) const noexcept { return pm_Data > other.pm_Data; }
		[[nodiscard]] inline bool operator>=(const duration& other) const noexcept { return pm_Data >= other.pm_Data; }
		[[nodiscard]] inline bool operator<(const duration& other) const noexcept { return pm_Data < other.pm_Data; }
		[[nodiscard]] inline bool operator<=(const duration& other) const noexcept { return pm_Data <= other.pm_Data; }
		[[nodiscard]] inline bool operator==(const duration& other) const noexcept { return pm_Data == other.pm_Data; }
		...
		inline duration& operator=(const DurationType& other) { pm_Data = T(other); return *this; }

		[[nodiscard]] inline operator duration<std::chrono::duration<float, std::ratio<60 * 60, 1>>>() const noexcept
		{
			return duration<std::chrono::duration<float, std::ratio<60 * 60, 1>>>(pm_Data);
		}
		[[nodiscard]] inline operator duration<std::chrono::duration<float, std::ratio<60, 1>>>() const noexcept
		{
			return duration<std::chrono::duration<float, std::ratio<60, 1>>>(pm_Data);
		}
		[[nodiscard]] inline operator duration<std::chrono::duration<float>>() const noexcept
		{
			return duration<std::chrono::duration<float>>(pm_Data);
		}
		[[nodiscard]] inline operator duration<std::chrono::duration<float, std::ratio<1, 1000>>>() const noexcept
		{
			return duration<std::chrono::duration<float, std::ratio<1, 1000>>>(pm_Data);
		}
		[[nodiscard]] inline operator duration<std::chrono::duration<float, std::ratio<1, 1000 * 1000>>>() const noexcept
		{
			return duration<std::chrono::duration<float, std::ratio<1, 1000 * 1000>>>(pm_Data);
		}
		[[nodiscard]] inline operator duration<std::chrono::duration<float, std::ratio<1, 1000 * 1000 * 1000>>>() const noexcept
		{
			return duration<std::chrono::duration<float, std::ratio<1, 1000 * 1000 * 1000>>>(pm_Data);
		}
		[[nodiscard]] inline static duration zero() noexcept { return duration(0); }
	...
	{
		friend class time;
		[[nodiscard]] inline timestep(seconds time = zx::seconds(0)) noexcept : pm_Step(time) {}
		template<typename DurationType>
		[[nodiscard]] inline timestep(const DurationType& copy) : pm_Step(copy) {}

		[[nodiscard]] float seconds() const noexcept { return pm_Step.count(); }
		[[nodiscard]] float miliseconds() const noexcept { return zx::milliseconds(pm_Step).count(); }
		[[nodiscard]] float fps() const noexcept { return 1.0f / pm_Step.count(); }

		[[nodiscard]] zx::seconds operator+(zx::seconds other) const noexcept { return pm_Step + other; }
		[[nodiscard]] zx::seconds operator-(zx::seconds other) const noexcept { return pm_Step - other; }
		[[nodiscard]] zx::seconds operator*(float other) const noexcept { return pm_Step * other; }
		[[nodiscard]] zx::seconds operator/(float other) const noexcept { return pm_Step / other; }

		template<typename DurationType>
		[[nodiscard]] inline operator DurationType() const noexcept { return pm_Step; }

		[[nodiscard]] inline operator hours() const noexcept { return pm_Step; }
		[[nodiscard]] inline operator minutes() const noexcept { return pm_Step; }
		[[nodiscard]] inline operator zx::seconds() const noexcept { return pm_Step; }
		[[nodiscard]] inline operator zx::milliseconds() const noexcept { return pm_Step; }
		[[nodiscard]] inline operator zx::nanoseconds() const noexcept { return pm_Step; }
		[[nodiscard]] inline operator float() const noexcept { return pm_Step.count(); }
